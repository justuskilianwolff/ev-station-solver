...
class LinearSolver(Solver):
    ...
    def add_new_dv_y_s(self, s: Sample, K: range):
        ...
        created_y_s = np.array([self.m.continuous_var(name=f"y_{s}_{i}_{k}", lb = 0)  for i in range(self.n_vehicles) if s.charging[i] for k in K])
    ...
    def add_new_dv_u_s(self, s: Sample, K: range):
        ...
        """
        # 
        created_u_s = np.array([self.m.binary_var(name=f"u_{s}_{i}_{k}") for i in range(self.n_vehicles) if s.charging[i] for k in K])
    ...
    def add_new_dv_x(self, K: range):
        ...
        created_x = np.array([self.m.continuous_var(name=f"x_{k}_{l}", lb = min_loc[l], ub = max_loc[l])  for k in K for l in range(2)])
    ...
    def add_distance_range_constraints(self, s: Sample, K: range) -> None:
        ...
        new_distance_range_constraints = self.m.add_constraints(
            ((s.ranges[i]**2)*self.u[s.index][i, k] + self.d_root[s.I_s[i],k] - s.ranges[i]**2  <= self.y[s.index][i, k] for i in s.I for k in K),
    ...
    def add_euclidian_distance_constraints(self, I: range, K: range) -> None:
        ...
        self.m.add_quadratic_constraints(
            self.d_root[i, k] >= (self.x[k,0] - self.vehicle_locations[i,0])**2 + (self.x[k,1] - self.vehicle_locations[i,1])**2 for i in I for k in K
    ...
    def get_drive_charge_cost(self, s: Sample, K: range):
        ...
        return self.drive_charge_cost_param * self.m.sum(
            self.m.sum(self.y[s.index][i,j]  for i in s.I for j in self.J) 
        )
    ...
    def build_xpress_model(self, solve_time):
        ...
        linearise_1 = [y[s.I_s[i],j,s.index] <= np.ceil((s.ranges[i]**2))*u[s.I_s[i],j,s.index]  for j in self.J for s in self.S for i in s.I]
        linearise_2 = [np.ceil((s.ranges[i]**2))*u[s.I_s[i],j,s.index]*100 + d_tilde[s.I_s[i],j] - np.ceil((s.ranges[i]**2))*100 <= y[s.I_s[i],j,s.index] for j in self.J for s in self.S for i in s.I]
        linearise_3 = [y[s.I_s[i],j,s.index]  <= d_tilde[s.I_s[i],j]   for j in self.J for s in self.S for i in s.I]
        
        two_norm = [d_tilde[i,j] >= (x[j,0] - self.vehicle_locations[i,0])**2 + (x[i,1] - self.vehicle_locations[i,1])**2 for j in self.J for i in range(self.n_vehicles)]
        ...
        obj += 365 / len(self.S) * self.drive_charge_cost_param * xp.Sum(
            xp.sqrt(y[i,j,s.index])   for j in self.J for s in self.S for i in s.I_s)
