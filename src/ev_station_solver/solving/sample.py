...
class Sample:
    def __init__(self, index: int, total_vehicle_locations: np.ndarray, coordinates_cl: np.ndarray):
        ...
        self.charging = ev_charging(ranges=ranges, charging_probabilites=charging_prob)

        # distance matrix of vehicles to cl
        self.index = index
        self.vehicle_locations: np.ndarray = total_vehicle_locations[self.charging]
        self.ranges: np.ndarray = ranges[self.charging]  # numpy 1D array of ranges of vehicles in sample
        ...
        self.I_s: list = [i for i in range(total_vehicle_locations.shape[0]) if self.charging[i]]
